...
\item
  ...
  \begin{itemize}
    ...
    the index of $\var{ract}$ as a key in the (unfiltered) map. The keys of the $\Wdrls$
    map are arranged in the order defined on the $\RewardAcnt$ type, which is a lex order
...
\begin{itemize}
  ...
  inputs by calling the non-native script validator function, $\fun{runPLCScript}$ (see
  \ref{sec:txinfo} for details).
  For native (timelock) scripts, it calls $\fun{evalTimelock}$
\end{itemize}
...
\begin{figure}[htb]
  \begin{align*}
    ...
    \nextdef
    & \fun{evalScripts} \in \Tx \times \seqof{(\ScriptNonNative \times \seqof{\Data} \times \ExUnits \times \CostMod)} \to \IsValid \\
    ...
    & \fun{evalScripts}~\var{tx}~((\var{sc}, \var{d}, \var{eu}, \var{cm});\Gamma) = \\
      & \begin{cases}
        ...
        \fun{evalTimelock}~\var{tx}~\var{sc} \land \fun{evalScripts}~\var{tx}~\Gamma & \var{sc}\in\ScriptNative
      \end{cases}
  \end{align*}
